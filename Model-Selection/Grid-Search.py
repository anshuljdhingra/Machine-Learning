# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 15:58:50 2017
...
classifier = SVC(kernel= 'rbf' , C = 1.0, gamma = 0.7, random_state=0)
...
# applying k-fold cross validation to evaulate model performance
from sklearn.model_selection import cross_val_score
accuracies = cross_val_score(estimator = classifier, X = X_train, y = y_train, cv = 10, n_jobs = -1)
mean_accuracy = accuracies.mean()
variance_accuracy = accuracies.std()

# applying grid search to find optimal values of hyperparameters for model's performance improvement
from sklearn.model_selection import GridSearchCV
'''parameters = [{'C' : [1,5,10,20,50,100], 'kernel' : ['linear']},
              {'C' : [1,5,10,20,50,100,500,1000], 'kernel' : ['rbf'], 'gamma' : [0.1, 0.2,0.3,0.5,0.01, 0.001]}]'''

parameters = [{'C' : [1,5,10,20,50,100], 'kernel' : ['linear']},
              {'C' : [1,5,10,20,50,100,500,1000], 'kernel' : ['rbf'], 'gamma' : [0.1, 0.2,0.3,0.5,0.6,0.7,0.8,0.9]}]
